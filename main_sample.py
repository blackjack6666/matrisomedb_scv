...
if __name__ == '__main__':
    # generating htmls for sample specific
    from glob import glob
    import pickle

    protein_dict = fasta_reader3('F:/matrisomedb2.0/mat.fasta')

    sample_data = pickle.load(open('F:/matrisomedb2_0_revise/sample_result9.data','rb'))  # sample name has illgal charcters
    print ('done loading sample data')
    # pq_sample_dict = {}
    # for sample in sample_data:
    #     print (sample)
    #     id_dict = {}
    #     id_protein_freq_dict = sample_data[sample]['freq']
    #     for prot in id_protein_freq_dict:
    #         freq_array = id_protein_freq_dict[prot]
    #         coverage = np.count_nonzero(freq_array)/len(freq_array)*100
    #         id_dict[prot] = [(coverage, (prot, protein_dict[prot]), freq_array.tolist(),True)]
    #     pq_sample_dict[sample] = id_dict
    # pickle.dump(pq_sample_dict,open('F:/matrisomedb2.0/pq_sample_dict.p','wb'),protocol=5)
    # pq_sample_dict = pickle.load(open('F:/matrisomedb2.0/pq_sample_dict.p','rb'))
    # print ('done loading pq')
    ...
    regex_dict = {'M\\[15\\.9949\\]':[0,255,0],'S\\[79\\.9663\\]':[235, 222, 52],'T\\[79\\.9663\\]':[235, 222, 52],
                  'Y\\[79\\.9663\\]':[235, 222, 52],'P\\[15\\.9949\\]':[26, 7, 237],'K\\[15\\.9949\\]':[26, 7, 237],
                  'R\\[0\\.9840\\]':[181, 2, 209],'n\\[42\\.0106\\]':[79, 78, 79]}
    ...
    # for sample in sample_data:
    #     print (sample)
    #     id_freq_dict = sample_data[sample]['freq']
    #     id_ptm_index_dict = sample_data[sample]['ptm']
    #     clean_id_ptm_idx_dict = {}
    #     # numpy array to list to feed json
    #     for prot in id_ptm_index_dict:
    #         dict1 = {}
    #         ptm_idx_dict = id_ptm_index_dict[prot]
    #         for ptm in ptm_idx_dict:
    #             index_list = ptm_idx_dict[ptm].tolist()
    #             dict1[ptm] = index_list
    #         clean_id_ptm_idx_dict[prot] = dict1
    #
    #     for protein in id_freq_dict:
    #         print (protein)
    #         # protein = 'O15230'
    ...
    #             output_path = 'F:/matrisomedb2.0/3dcov_htmls/' +sample+'_'+ protein + '_3dcov.html'
    ...
    #             freq_array = id_freq_dict[protein]
    #             # pdb_file = r'D:\data\alphafold_pdb\UP000000589_10090_MOUSE/AF-Q99MQ5-F1-model_v1.pdb'
    #
    #             try:
    #                 dump_dict = show_cov_3d_v2(protein, 'AAA', pdb_file, np.array(freq_array).astype(np.int32),
    ...
    #                 pq_dump = [(np.count_nonzero(freq_array)/len(freq_array)*100, (protein, protein_dict[protein]), freq_array.tolist(),True)]
    ...
    sample = 'Symptomatic carotid stenosis (Insoluble ECM protein fraction)'
    proteinid = 'P24821'
    id_freq_dict = sample_data[sample]['freq']
    id_ptm_index_dict = sample_data[sample]['ptm']

    freq_array = id_freq_dict[proteinid]
    ptm_ind_dict = id_ptm_index_dict[proteinid]

    clean_ptm_ind_dict = {proteinid:{ptm:ptm_ind_dict[ptm].tolist() for ptm in ptm_ind_dict}}

    pdb_file = 'F:/matrisomedb2.0/mat_alphafold_pdbs/AF-' + proteinid + '-F1-model_v1.pdb'
    output_path = 'F:/matrisomedb2.0/3dcov_htmls/' + sample + '_' + proteinid + '_3dcov.html'

    regex_dict = {each: regex_dict[each] for each in regex_dict if each in ptm_ind_dict}

    # pdb_file = r'D:\data\alphafold_pdb\UP000000589_10090_MOUSE/AF-Q99MQ5-F1-model_v1.pdb'

    dump_dict = show_cov_3d_v2(proteinid, 'AAA', pdb_file, np.array(freq_array).astype(np.int32),
                               id_ptm_idx_dict=clean_ptm_ind_dict[proteinid],
                               regex_color_dict=regex_dict,
                               background_color=str(16777215))
    ...
    soup = Soup(view_file.read(), 'lxml')
    js_data = soup.find(id='data')
    pq_dump = [(np.count_nonzero(freq_array) / len(freq_array) * 100, (proteinid, protein_dict[proteinid]),
                freq_array.tolist(), True)]
    js_data.string = 'let id_ptm_idx_dict = ' + json.dumps(clean_ptm_ind_dict) + ';' + \
                     'let regex_dict = ' + json.dumps(regex_dict) + ';' + \
                     'let background_color = ' + str(16777215) + ';' + \
                     'let pq = ' + json.dumps(pq_dump) + ';'
    pdb_str = soup.find(id='glmol01_src')
